...
//	Serial.println(pdu_opcode_to_string(bhs.get_opcode()).c_str());
	...
	switch(bhs.get_opcode()) {
		...
		case iscsi_pdu_bhs::iscsi_bhs_opcode::o_scsi_data_out:
			...
			break;
		default:
			errlog("server::receive_pdu: opcode %02xh not implemented\n", bhs.get_opcode());
...
bool server::push_response(com_client *const cc, session *const ses, iscsi_pdu_bhs *const pdu, iscsi_response_parameters *const parameters)
{
	...
	if (pdu->get_opcode() == iscsi_pdu_bhs::iscsi_bhs_opcode::o_scsi_data_out) {
		...
		if (session == nullptr) {
			errlog("server::push_response: DATA-OUT PDU references unknown TTT (%08x)\n", TTT);
			if (data.has_value())
			...
			if (rc != scsi::rw_ok) {
				errlog("server::push_response: DATA-OUT problem writing to backend (%d)\n", rc);
				return false;
			}
	...
	for(auto & pdu_out: response_set.value().responses) {
		for(auto & blobs: pdu_out->get()) {
			...
			if (ok) {
				...
				if (!ok)
					errlog("server::push_response: sending PDU to peer failed (%s)\n", strerror(errno));
				bytes_send += blobs.n;
			}
	...
	if (response_set.value().to_stream.has_value()) {
		...
		for(uint32_t block_nr = 0; block_nr < stream_parameters.n_sectors;) {
			...
			if (buffer.n > 0) {
				...
				if (rc != scsi::rw_ok) {
					errlog("server::push_response: reading %u block(s) %zu from backend failed (%d)\n", n_left, size_t(cur_block_nr), rc);
					ok = false;
					break;
				}
			}
			...
			if (out.n == 0) {  // gen_data_in_pdu could not allocate memory
				...
				buffer.n = 512;
				buffer.data = new uint8_t[buffer.n]();
				errlog("Low on memory: %zu bytes failed\r\n", buffer.n);
			}
			else {
				if (cc->send(out.data, out.n) == false) {
					delete [] out.data;
					errlog("server::push_response: problem sending block %zu to initiator\n", size_t(cur_block_nr));
					ok = false;
					break;
				}
...
iscsi_response_parameters *server::select_parameters(iscsi_pdu_bhs *const pdu, session *const ses, scsi *const sd)
{
	switch(pdu->get_opcode()) {
		...
		default:
			errlog("server::select_parameters: opcode %02xh not implemented\n", pdu->get_opcode());
			break;
	}
...
void server::handler()
{
	...
	while(!stop) {
		...
		if (cc == nullptr) {
			errlog("server::handler: accept() failed: %s\n", strerror(errno));
			continue;
		}
